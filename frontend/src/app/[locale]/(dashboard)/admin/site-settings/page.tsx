...
function SaveButton({ saving, saved, onClick }: { saving: boolean; saved: boolean; onClick: () => void }) {
    return (
        <Button onClick={onClick} disabled={saving} size="sm" variant="outline" className="gap-2">
            {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : saved ? <Check className="h-3.5 w-3.5 text-green-600" /> : null}
...
export default function SiteSettingsPage() {
    ...
    const [savingMap, setSavingMap] = useState<Record<SectionKey, boolean>>({
        branding: false, colors: false, social: false, identity: false,
        google: false, privacy: false, cookies: false,
    })
    ...
    const saveSection = async (section: SectionKey, fn: () => Promise<void>) => {
        ...
        try {
            ...
            setTimeout(() => setSavedMap(p => ({ ...p, [section]: false })), 2500)
        } catch (err) {
            console.error(`[SiteSettings:${section}] Save failed:`, err)
    ...
    const handleSaveBranding = () => saveSection('branding', async () => {
        const fd = new FormData()
        if (logoFile) fd.append('logo', logoFile)
        if (logoSmallFile) fd.append('logo_small', logoSmallFile)
        if (faviconFile) fd.append('favicon', faviconFile)
        if (!logoFile && !logoSmallFile && !faviconFile) {
            toast({ title: 'Sin cambios', description: 'Selecciona al menos una imagen para subir.' })
            return
        }
        const data = await siteSettingsApi.update(fd)
        setLogoFile(null)
        setLogoSmallFile(null)
        setFaviconFile(null)
        if (data.favicon_16_url) {
            setFaviconVariants({
                favicon_16_url: data.favicon_16_url,
                favicon_32_url: data.favicon_32_url,
                apple_touch_icon_url: data.apple_touch_icon_url,
    ...
    const handleSaveColors = () => saveSection('colors', async () => {
        await siteSettingsApi.updateJson({
            safari_pinned_tab_color: safariPinnedTabColor,
            ms_tile_color: msTileColor,
            theme_color: themeColor,
        })
    })

    const handleSaveSocial = () => saveSection('social', async () => {
        if (ogImageFile) {
            const fd = new FormData()
            fd.append('og_image', ogImageFile)
            fd.append('og_description', ogDescription)
            fd.append('twitter_handle', twitterHandle)
    ...
    const handleSaveIdentity = () => saveSection('identity', async () => {
        await siteSettingsApi.updateJson({
            site_name: siteName,
            tagline: tagline,
        })
    ...
    const handleSaveGoogle = () => saveSection('google', async () => {
        await siteSettingsApi.updateJson({
            ga_id: gaId,
            gtm_id: gtmId,
            gsc_id: gscId,
    ...
    const handleSavePrivacy = () => saveSection('privacy', async () => {
        await siteSettingsApi.updateJson({
    ...
    const handleSaveCookies = () => saveSection('cookies', async () => {
        await siteSettingsApi.updateJson({
            cookie_consent_enabled: cookieConsentEnabled,
            cookies_analytics_enabled: cookiesAnalytics,
            cookies_marketing_enabled: cookiesMarketing,
            cookies_functional_enabled: cookiesFunctional,
            compliance_gdpr: complianceGdpr,
            compliance_lgpd: complianceLgpd,
            compliance_hipaa: complianceHipaa,
            compliance_ccpa: complianceCcpa,
        ...
        <div className="p-6 space-y-6 max-w-3xl">
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.branding} saved={savedMap.branding} onClick={handleSaveBranding} />
                </CardHeader>
                <CardContent>
                    <div className="grid md:grid-cols-3 gap-6">
                        ...
                        {/* Logo Small */}
                        <div className="space-y-4">
                            <p className="text-sm font-medium">Logo Reducido</p>
                            <div className="flex items-center justify-center h-20 border rounded-lg bg-muted/30 overflow-hidden">
                                {logoSmallPreview ? (
                                    <img src={logoSmallPreview} alt="Logo small preview" className="h-10 w-10 object-contain" />
                                ) : (
                                    <span className="text-sm text-muted-foreground text-center px-2">Sin logo reducido</span>
                                )}
                            </div>
                            <input ref={logoSmallInputRef} type="file" accept="image/*" className="hidden" onChange={handleLogoSmallChange} />
                            <Button variant="outline" className="w-full gap-2" onClick={() => logoSmallInputRef.current?.click()}>
                                <Upload className="h-4 w-4" /> Subir imagen
                            </Button>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-center h-20 border rounded-lg bg-muted/30">
                                {faviconPreview ? (
                                    <img src={faviconPreview} alt="Favicon preview" className="h-12 w-12 object-contain" />
                                ) : (
                            ...
                            <Button variant="outline" className="w-full gap-2" onClick={() => faviconInputRef.current?.click()}>
                                <Upload className="h-4 w-4" /> Subir .ico/.png
                            </Button>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.colors} saved={savedMap.colors} onClick={handleSaveColors} />
                </CardHeader>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <div className="flex gap-2">
                                <input type="color" id="theme_color" value={themeColor} onChange={e => setThemeColor(e.target.value)} className="h-9 w-9 rounded border cursor-pointer" />
                                <Input value={themeColor} onChange={e => setThemeColor(e.target.value)} className="font-mono text-xs" maxLength={7} />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="ms_tile_color">MS Tile Color</Label>
                            <div className="flex gap-2">
                                <input type="color" id="ms_tile_color" value={msTileColor} onChange={e => setMsTileColor(e.target.value)} className="h-9 w-9 rounded border cursor-pointer" />
                                <Input value={msTileColor} onChange={e => setMsTileColor(e.target.value)} className="font-mono text-xs" maxLength={7} />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="safari_pinned_tab_color">Safari Pinned Tab</Label>
                            <div className="flex gap-2">
                                <input type="color" id="safari_pinned_tab_color" value={safariPinnedTabColor} onChange={e => setSafariPinnedTabColor(e.target.value)} className="h-9 w-9 rounded border cursor-pointer" />
                                <Input value={safariPinnedTabColor} onChange={e => setSafariPinnedTabColor(e.target.value)} className="font-mono text-xs" maxLength={7} />
                            </div>
                        </div>
                    </div>
                    <p className="text-[10px] text-muted-foreground">
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.social} saved={savedMap.social} onClick={handleSaveSocial} />
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-1.5">
                        ...
                        <div className="flex items-center justify-center h-32 border rounded-lg bg-muted/30 overflow-hidden">
                            {ogImagePreview ? (
                                <img src={ogImagePreview} alt="OG image preview" className="h-full w-full object-cover" />
                            ) : (
                                <span className="text-sm text-muted-foreground">Sin imagen OG</span>
                            )}
                        </div>
                        <input ref={ogImageInputRef} type="file" accept="image/*" className="hidden" onChange={handleOgImageChange} />
                        <Button variant="outline" className="w-full gap-2" onClick={() => ogImageInputRef.current?.click()}>
                            <Upload className="h-4 w-4" /> Subir imagen (1200x630 recomendado)
                        </Button>
                    </div>
                    <div className="space-y-1.5">
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle className="text-base">Nombre y Tagline</CardTitle>
                    <SaveButton saving={savingMap.identity} saved={savedMap.identity} onClick={handleSaveIdentity} />
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="site_name">Nombre del sitio</Label>
                        <Input id="site_name" value={siteName} onChange={e => setSiteName(e.target.value)} placeholder="BVS" />
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="tagline">Tagline</Label>
                        <Input id="tagline" value={tagline} onChange={e => setTagline(e.target.value)} placeholder="Plataforma digital de conocimiento" />
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.google} saved={savedMap.google} onClick={handleSaveGoogle} />
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="ga_id">Google Analytics ID</Label>
                        <Input id="ga_id" value={gaId} onChange={e => setGaId(e.target.value)} placeholder="G-XXXXXXXXXX" />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Input id="gtm_id" value={gtmId} onChange={e => setGtmId(e.target.value)} placeholder="GTM-XXXXXXX" />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Input id="gsc_id" value={gscId} onChange={e => setGscId(e.target.value)} placeholder="Código de verificación meta..." />
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.privacy} saved={savedMap.privacy} onClick={handleSavePrivacy} />
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="privacy_policy_url">URL Política de Privacidad</Label>
                        <Input id="privacy_policy_url" type="url" value={privacyPolicyUrl} onChange={e => setPrivacyPolicyUrl(e.target.value)} placeholder="https://..." />
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="terms_of_service_url">URL Términos de Servicio</Label>
                        <Input id="terms_of_service_url" type="url" value={termsOfServiceUrl} onChange={e => setTermsOfServiceUrl(e.target.value)} placeholder="https://..." />
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="cookie_policy_url">URL Política de Cookies</Label>
                        <Input id="cookie_policy_url" type="url" value={cookiePolicyUrl} onChange={e => setCookiePolicyUrl(e.target.value)} placeholder="https://..." />
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <SaveButton saving={savingMap.cookies} saved={savedMap.cookies} onClick={handleSaveCookies} />
                </CardHeader>
                    ...
                    {cookieConsentEnabled && (
                        <>
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <Checkbox id="cookies_analytics" checked={cookiesAnalytics} onCheckedChange={(v) => setCookiesAnalytics(v === true)} />
                                    <Label htmlFor="cookies_analytics" className="text-sm">Analítica</Label>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Checkbox id="cookies_marketing" checked={cookiesMarketing} onCheckedChange={(v) => setCookiesMarketing(v === true)} />
                                    <Label htmlFor="cookies_marketing" className="text-sm">Marketing</Label>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Checkbox id="cookies_functional" checked={cookiesFunctional} onCheckedChange={(v) => setCookiesFunctional(v === true)} />
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <Checkbox id="compliance_gdpr" checked={complianceGdpr} onCheckedChange={(v) => setComplianceGdpr(v === true)} />
                                    <Label htmlFor="compliance_gdpr" className="text-sm">GDPR (Unión Europea)</Label>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Checkbox id="compliance_lgpd" checked={complianceLgpd} onCheckedChange={(v) => setComplianceLgpd(v === true)} />
                                    <Label htmlFor="compliance_lgpd" className="text-sm">LGPD (Brasil)</Label>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Checkbox id="compliance_hipaa" checked={complianceHipaa} onCheckedChange={(v) => setComplianceHipaa(v === true)} />
                                    <Label htmlFor="compliance_hipaa" className="text-sm">HIPAA (Salud - EE.UU.)</Label>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Checkbox id="compliance_ccpa" checked={complianceCcpa} onCheckedChange={(v) => setComplianceCcpa(v === true)} />
                            ...
                            <div className="space-y-4 border-t pt-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input id="cookie_banner_title" value={cookieBannerTitle} onChange={e => setCookieBannerTitle(e.target.value)} placeholder="Utilizamos cookies" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label htmlFor="cookie_banner_description">Descripción del banner</Label>
                                    <Textarea id="cookie_banner_description" value={cookieBannerDescription} onChange={e => setCookieBannerDescription(e.target.value)} placeholder="Este sitio utiliza cookies..." rows={3} />
                                </div>
                            </div>
                        </>
                    )}
