...
echo "========================================"
echo ""
echo "Este script solucionará ambos errores:"
echo "1. DOMMatrix is not defined (frontend)"
echo "2. Error al iniciar sesión de lectura (backend)"
echo ""
...
docker compose exec -T backend python manage.py showmigrations content | grep -E "0004|0005|0006"
...
echo "2. O abre en ventana incógnita"
...
print('\nLibros con PDF disponibles:')
for b in books[:5]:
    print(f'  ID {b.id}: {b.title} - http://localhost:3000/reader/{b.id}')
"

echo ""
echo "Si aún ves errores, revisa:"
